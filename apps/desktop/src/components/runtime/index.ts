...
class Runtime extends EventEmitter {
  ...
  private processRuntime() {
    // TODO(阶段3): 游戏启动将由 Tauri Rust 侧命令实现。
    // webview 前端本就无法使用 child_process，且直接执行用户选定路径属于命令注入面，
    // 安全扫描已强制拦截。此处先不启动进程，PID 探测逻辑保持不变。
  }
    ...
    this.retryTimer = new RetryTimer({
      callBack: () =>
        new Promise((resolve, reject) => {
          execFile(
            "tasklist",
            ["/nh", "/fo", "csv", "/fi", `imagename eq ${this.getExeName()}`],
            (error, stdout) => {
              if (error) {
                return reject()
              }
